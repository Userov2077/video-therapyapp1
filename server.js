...
files.forEach(f => {
    try { 
        const filePath = `${dataPath}${f}.json`;
        if (fs.existsSync(filePath)) {
            data[f] = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        } else {
            data[f] = [];
...
function saveData() { 
    files.forEach(f => {
        try {
            fs.writeFileSync(`${dataPath}${f}.json`, JSON.stringify(data[f], null, 2), 'utf8');
        } catch(e) { console.error(`Ошибка сохранения ${f}:`, e); }
    });
}
...
app.post('/api/upload-recording', upload.single('recording'), (req, res) => {
    ...
    saveData();
    res.json({ success: true, recordingUrl: recordingUrl });
});

// API Загрузка для постов
...
app.post('/api/certificate', upload.single('file'), (req, res) => {
    ...
    const user = getUser(userId);
    if (!user || user.role !== 'psychologist') return res.json({ success: false, error: 'Только для психологов' });
    if (!req.file) return res.json({ success: false, error: 'Файл не загружен' });
...
app.get('/api/posts', (req, res) => {
    ...
    const enriched = posts.map(post => {
        ...
        return { ...post, author: { id: author.id, fullName: author.fullName, avatar: author.avatar }, likes: likes.length, userLiked: false, comments };
    });
...
app.post('/api/posts/:id/comment', (req, res) => {
    const { userId, text } = req.body;
    const postId = req.params.id;
    const comment = { id: Date.now().toString(), postId, authorId: userId, text, createdAt: new Date().toISOString() };
...
app.post('/api/appointment', (req, res) => {
    ...
    if (!daySchedule || !daySchedule.includes(time)) {
        return res.json({ success: false, error: 'Это время уже занято или психолог не работает' });
    }
    ...
    psychologist.clients = psychologist.clients || [];
    psychologist.clients.push({
        clientId, clientName: client.fullName, clientAvatar: client.avatar, appointmentId: appointment.id, date, time, status: 'pending', roomId
    });
    psychologist.notifications.unshift({
        id: Date.now().toString(),
    ...
    saveData();
    res.json({ success: true, appointment, roomId });
});
...
app.post('/api/appointment/confirm', (req, res) => {
    ...
    client.notifications.unshift({
        ...
        appointmentId, roomId: appointment.roomId, read: false, createdAt: new Date().toISOString()
    });
    saveData();
    res.json({ success: true, roomId: appointment.roomId });
});

app.get('/api/appointments/psychologist/:id', (req, res) => {
    const psychologist = getUser(req.params.id);
    if (!psychologist) return res.json({ success: false });
    res.json({ success: true, appointments: psychologist.clients || [] });
});
...
app.get('/api/messages/:userId', (req, res) => {
    ...
    let contactIds = [];
    if (user.role === 'client') {
        contactIds = data.users.filter(u => u.role === 'psychologist').map(u => u.id);
    } else {
        const otherPsychologists = data.users.filter(u => u.role === 'psychologist' && u.id !== userId).map(u => u.id);
        const clientFromAppointments = (user.clients || []).map(c => c.clientId);
        contactIds = [...new Set([...otherPsychologists, ...clientFromAppointments])];
    }
...
app.post('/api/messages/read', (req, res) => {
    ...
    const messages = data.messages.filter(m => m.to === userId && m.from === fromUserId && !m.isRead);
...
app.get('/api/psychologists', (req, res) => {
    const psychologists = data.users.filter(u => u.role === 'psychologist').map(({ password, ...u }) => u);
...
io.on('connection', (socket) => {
    ...
    socket.on('stop-recording', async (recordingData) => {
        const room = activeRooms.get(socket.roomId);
        if (room && room.recording) {
            ...
            if (recordingData && recordingData.url) {
                const recording = {
                    ...
                    duration: Date.now() - room.recordingStartTime,
                    createdAt: new Date().toISOString()
                };
                data.recordings.push(recording);
                saveData();
